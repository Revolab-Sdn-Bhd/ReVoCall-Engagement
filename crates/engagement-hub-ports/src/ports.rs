...
pub trait RegistryPort: Send + Sync {
    async fn resolve_snapshot(&self, req: ResolveSnapshotReq) -> Result<ResolvedSnapshot, RegistryError>;
...
pub trait JourneyManagerPort: Send + Sync {
    ...
    async fn cancel_execution(&self, ref_: &ExecutionRef, reason: CancelReason) -> Result<(), JmError>;
    async fn get_execution_timeline(&self, ref_: &ExecutionRef, opts: TimelineOpts) -> Result<Timeline, JmError>;
}

#[async_trait]
pub trait VoiceManagerPort: Send + Sync {
    async fn start_voice_session(&self, req: StartVoiceSessionReq) -> Result<VoiceSessionRef, VmError>;
    async fn stop_voice_session(&self, ref_: &VoiceSessionRef, mode: StopMode) -> Result<(), VmError>;
...
pub trait PostCallPort: Send + Sync {
    ...
    async fn get_output_extraction(&self, eng: &EngagementId) -> Result<OutputExtraction, PostCallError>;
    async fn list_agent_call_logs(&self, req: ListAgentCallLogsReq) -> Result<Page<CallLog>, PostCallError>;
    async fn list_org_call_logs(&self, req: ListOrgCallLogsReq) -> Result<Page<CallLog>, PostCallError>;
}

#[async_trait]
pub trait AnalyticsPort: Send + Sync {
    async fn get_agent_analytics(&self, req: GetAgentAnalyticsReq) -> Result<Analytics, AnalyticsError>;
    async fn get_agent_metrics(&self, req: GetAgentMetricsReq) -> Result<Metrics, AnalyticsError>;
    async fn get_org_analytics(&self, req: GetOrgAnalyticsReq) -> Result<Analytics, AnalyticsError>;
